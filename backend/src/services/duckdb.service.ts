...
export class DuckDBService {
  ...
  async runQuery(filePath: string, sql: string, params: unknown[] = []): Promise<Row[]> {
    ...
    try {
      ...
      throw new DuckDbError(message, { cause: err as Error });
    } finally {
      await close(db);
    }
  }
...
/**
 * Typed wrapper around raw DuckDB errors so callers (AnalyticsService) can
 * distinguish DuckDB failures from other runtime errors.
 */
